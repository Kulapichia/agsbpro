...
#### 问题一：`hysteria-fileserver.service` 启动失败 (端口占用)

如果你在执行 `sudo systemctl status hysteria-fileserver.service` 后，看到服务状态不是 `active (running)`，而是处于不断重启的 `activating` 状态，通常是由于端口被占用导致的。
...
2.  **第二步：查找并终止占用端口的进程**
    ...
    ```
    这个命令会显示出监听 8080 端口的进程及其 PID (进程ID)。

3.  **第三步：终止旧进程**
  
    假设上一步找到的 PID 是 `12345`，使用 `kill` 命令强制结束它。
...
#### 问题二：`hysteria-server.service` 启动失败 (配置文件或证书缺失)

如果主服务启动失败，很可能是因为一键安装脚本没有完整执行，导致 `config.json` 或证书文件缺失。
...
2.  **第二步：重新运行安装脚本以生成配置文件**
    ...
    python3 nginx-hysteria2.py install --simple --port-range 28888-29999 --enable-bbr
    ```
    > **注意**：忽略此命令最后可能出现的任何报错，我们的目的只是补全文件。
...
3.  **第三步：检查文件是否已生成**
    ...
    ```
    > 确认 `config.json`, `server.crt`, `server.key` 文件都已存在。

4.  **第四步：使用 Systemd 重新启动服务**
