...
def create_full_nginx_config():
    ...
    for service, data in shared_config.items():
        ...
        # 证书 Map
        cert_path = data.get('cert_path', f"/opt/{service}.pem") # 提供一个备用路径
        key_path = data.get('key_path', f"/opt/{service}.key")
        cert_map_lines.append(f"        {domain}    {cert_path};")
        key_map_lines.append(f"        {domain}    {key_path};")

        # Location 逻辑
        if service == 'hysteria2':
             locations_443.append(f'if ($host = "{domain}" or $host = $server_addr) {{ root {data["web_root"]}; index index.html; try_files $uri $uri/ =404; }}')
        elif service == 'vpn_fileserver':
            # vpn服务也需要证书，但通常和hysteria2共享
            vpn_cert_path = shared_config.get('hysteria2', {}).get('cert_path', cert_path)
            vpn_key_path = shared_config.get('hysteria2', {}).get('key_path', key_path)
            other_servers.append(f"""
    server {{
        listen 443 ssl http2;
        server_name {domain};
        ssl_certificate {vpn_cert_path};
        ssl_certificate_key {vpn_key_path};
        location / {{
            ...
            proxy_set_header Host $http_host;
             ...
             locations_443.append(f"""if ($host = "{domain}") {{
    ...
    elif cert_map_lines: # 使用第一个找到的证书作为默认
...
'''
    ...
    nginx_snippet = f"""
# ArgoSB Nginx 配置片段 (由 upload-agsb-v2.py 生成)
